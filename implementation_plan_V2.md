...
*   **Dia 25: Human-in-the-Loop: Revisão, Edição e Exportação de PDF**
    *   **Objetivo:** Desenvolver uma área interativa no dashboard onde você pode editar as sugestões da IA e clicar em um botão para exportar seu currículo otimizado formatado em HTML/PDF elegante. Adicionalmente, criar um segundo tipo de exportação: um **PDF de Lista de Vagas com diagramação padrão fixa**, gerado automaticamente a partir dos dados do banco SQLite.
    *   **Conceitos:** UX de ferramentas de edição de IA (co-criação humano-máquina), bibliotecas de geração de PDF no frontend/backend (ex: `html2pdf.js` ou PDF print styles), template HTML/CSS com layout fixo e preenchimento dinâmico de dados.
    *   **Exercício 1 (Currículo):** Criar o editor rich-text leve no modal e programar a exportação do currículo customizado no padrão estético senior.
    *   **Exercício 2 (PDF de Vagas — Feature nova):** Criar um template HTML com a sua diagramação padrão (que você fornecerá) contendo seções fixas como cabeçalho, tabela de vagas e rodapé. Programar um endpoint na API Express (`GET /export/jobs-pdf`) que consulta as vagas do SQLite, injeta os dados no template e gera o PDF via `html2pdf.js` ou Puppeteer local. Adicionar um botão "Exportar Lista de Vagas PDF" no dashboard que aciona esse endpoint.
    *   **Entregável:** Função de exportação de currículos customizados + template de PDF de vagas com diagramação padrão, ambos funcionando no Dashboard.
    *   **Tempo estimado:** 6h
    *   **Critério de Sucesso:** (1) Conseguir editar as sugestões do Ollama no dashboard e exportar um currículo em formato A4 PDF impecável em menos de 1 minuto. (2) Clicar em "Exportar Lista de Vagas PDF" e receber um documento formatado com a diagramação padrão definida, preenchido automaticamente com todas as vagas ativas do banco.

*   **Dia 26: Telemetria, Custos Locais, Monitoramento de Erros e Relatório Quinzenal de Keywords**
    *   **Objetivo:** Monitorar o consumo do ecossistema local (latências, tempos de resposta de IA, taxas de erro do scraper) para comprovar a eficiência técnica do sistema. Adicionalmente, criar um **relatório automático quinzenal** que analisa as palavras-chave e competências que mais se repetem nas vagas capturadas, gerando inteligência de mercado personalizada.
    *   **Conceitos:** Métricas de performance de IA (tempo até o primeiro token - TTFT, tokens por segundo), monitoramento de logs de rotas Express, análise de frequência de termos em SQL (`GROUP BY` + `COUNT`), agendamento cron no n8n, visualização de dados textuais (ranking de keywords ou word cloud).
    *   **Exercício 1 (Telemetria):** Criar um painel de "Status do Ecossistema" no seu dashboard mostrando métricas reais de tempo de processamento e volume de dados processados.
    *   **Exercício 2 (Relatório de Keywords — Feature nova):** Criar uma query SQL que agrupa e conta os termos e skills mais frequentes extraídos das vagas nos últimos 15 dias (a partir do campo JSON estruturado salvo pelo Ollama no SQLite). Criar um endpoint `GET /reports/keywords` na API Express que retorna esse ranking. No dashboard, adicionar uma seção visual "Tendências de Mercado" com os top 20 termos em formato de barras ou lista ranqueada. No n8n, configurar um agendamento cron quinzenal (`0 9 1,16 * *`) que dispara a query, salva o snapshot do relatório no banco e, opcionalmente, envia um resumo formatado via Discord/Slack.
    *   **Entregável:** Sistema de logs e telemetria integrado na tela do painel + relatório quinzenal automatizado de keywords com visualização no dashboard e notificação via Discord/Slack.
    *   **Tempo estimado:** 5h
    *   **Critério de Sucesso:** (1) Ver no dashboard as estatísticas reais de performance local. (2) Visualizar o ranking das palavras-chave mais exigidas nas vagas dos últimos 15 dias diretamente no dashboard, e receber o relatório automaticamente via notificação no Discord/Slack na data agendada.
